...
if not st.session_state.game_active:
    ...
    if len(active_players) <= 1:
        ...
        if st.button("次のラウンドへ"):
            ...
            st.session_state.show_results = False
            st.rerun()
コードは注意してご使用ください。改善ポイントプログレスバーの追加: 各プレイヤーのポイントを「残りライフ」としてバーで表示しました。バーが減るほど脱落（-5pt）に近づいているのが一目でわかります。色の変化: ポイントが減り、脱落しそうなプレイヤー（-4pt以下）はサイドバーの文字が赤色になります。脱落者は灰色になります。入力者の明示: 「今誰が入力しているか」を中央に大きく表示するようにしました。これで誰が有利で、誰がピンチなのか、ゲームの状況がよりはっきりわかるはずです。このスコアボードの「バーの色」や「表示する項目」について、もっとこうしたいという希望はありますか？AI の回答には間違いが含まれている場合があります。 詳細名前を0.8倍サバイバルではなく天秤に変えてくださいタイトルを「0.8倍サバイバル」から「天秤」へと変更し、それに合わせて全体の雰囲気も少し厳かな「天秤」のイメージに統一しました。pythonimport streamlit as st
import pandas as pd
